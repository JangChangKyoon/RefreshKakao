...
import com.example.refreshtokenandkakao.common.jwt.*;
...
//        UserValidator.validateInputUsername(requestDto);
    ...
    public Map<String, Object> login(LoginDto loginDto) {
        ...
        Users user = userRepository.findByUsername(loginDto.getUsername()).orElseThrow(
    ...
    public void deleteRefreshToken(TokenRequestDto tokenRequestDto) {
        Users user = userRepository.findById(tokenRequestDto.getUserId()).orElseThrow(
    ...
    public TokenDto SignupUserCheck(Long kakaoId) {
        ...
        Users loginUser = userRepository.findByKakaoId(kakaoId).orElse(null);
    ...
    //추가 정보 기입
    @Transactional
    public TokenDto addInfo(SignupRequestDto requestDto, Users user1) {
        // 닉네임 중복 확인
        String nickname = requestDto.getNickname();
        if (userRepository.existsByNickname(nickname)) {
            throw new CustomException(ErrorCode.SIGNUP_NICKNAME_DUPLICATE_CHECK);
        ...
        Users user = userRepository.findById(user1.getId())
    ...
    public void deleteUser() {
        Users foundUser = commonService.getUser();
